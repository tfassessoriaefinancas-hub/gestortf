...
type MemberRow={id:number|string;name?:string|null;email?:string|null;login?:string|null;permissions_json?:string|null;active?:number|boolean|null;partner_id?:number|string|null;partner_name?:string|null};
...
const mapMember=(row:MemberRow)=>({
  id:Number(row.id),name:String(row.name||''),email:String(row.email||''),login:String(row.login||''),
...
export async function GET(){
  ...
  const rows=await env.DB.prepare('SELECT au.id,au.name,au.email,au.login,au.permissions_json,au.active,au.partner_id,p.name partner_name FROM access_users au LEFT JOIN partners p ON p.id=au.partner_id WHERE au.owner_id=? ORDER BY au.active DESC,lower(au.name)').bind(owner.ownerKey).all<MemberRow>();
...
export async function PATCH(request:Request){
  ...
  const name=String(body.name??current.name).trim(),email=String(body.email??current.email).trim().toLowerCase(),login=String(body.login??current.login??'').trim();
  ...
  try{
    const row=await persistMember(env.DB.prepare('UPDATE access_users SET name=?,email=?,login=?,permissions_json=?,partner_id=?,active=?,updated_at=? WHERE id=? AND owner_id=? RETURNING id,name,email,login,permissions_json,partner_id,active').bind(name,email,login||null,JSON.stringify(partnerId?normalizePermissions(['inicio','parceiros','relatorios']):permissions),partnerId,active?1:0,Date.now(),id,owner.ownerKey),password);
...
export async function DELETE(request:Request){
  ...
  const current=await env.DB.prepare('SELECT id,name,email,login,permissions_json,partner_id,active FROM access_users WHERE id=? AND owner_id=?').bind(id,owner.ownerKey).first<MemberRow>();
  if(!current)return json({error:'Usuário não encontrado.'},404);
  const row=await persistMember(env.DB.prepare('UPDATE access_users SET active=0,updated_at=? WHERE id=? AND owner_id=? RETURNING id,name,email,login,permissions_json,partner_id,active').bind(Date.now(),id,owner.ownerKey), '');
  return json({member:mapMember(row)});
}
...
async function syncMemberAccount(client: PoolClient, member: ReturnType<typeof mapMember>, password: string) {
  ...
  const statements = [env.DB.prepare(`INSERT INTO users (id,email,login,name,role,active,created_at,updated_at)
    ...
    .bind(id, member.email, member.login || null, member.name, member.active ? 1 : 0, now, now)];
