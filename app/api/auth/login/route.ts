...
export async function POST(request: Request) {
  ...
  if (!identifier || !password || password.length > 256) return Response.json({ error: 'Informe CPF ou e-mail e senha válidos.' }, { status: 400 });
  const account = await env.DB.prepare(`SELECT u.id,c.password_hash FROM users u JOIN auth_credentials c ON c.user_id=u.id
    WHERE ${identifier.kind === 'cpf' ? 'u.cpf' : 'lower(u.email)'}=? AND u.active=1 AND u.deleted_at IS NULL`).bind(identifier.value).first<{ id: string; password_hash: string }>();
  ...
  if (!account || !valid) return Response.json({ error: 'CPF, e-mail ou senha incorretos.' }, { status: 401 });
