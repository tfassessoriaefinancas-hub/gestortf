...
const normalizedGgAdjustment=(item:any)=>({operationId:item.operation_id,ilaRate:Number(item.ila_rate_bps||0)===2600?26.6:Number(item.ila_rate_bps||0)/100,invoiceRate:Number(item.invoice_rate_bps||0)/100,tfShare:Number(item.tf_share_bps??5000)/100});
...
export async function GET(){
  ...
  const adjustmentSql="SELECT a.partner_id,a.operation_id,a.ila_rate_bps,a.invoice_rate_bps,a.tf_share_bps FROM partner_operation_adjustments a JOIN operations o ON o.id=a.operation_id AND o.deleted_at IS NULL";
  ...
  const operations=await readOperations(env.DB,user);
  return json({partners:rows.results.map(row=>({id:row.id,name:row.name,taxRate:(row.tax_rate_bps||0)/100,invoiceRate:(row.invoice_rate_bps||0)/100,tfShare:(row.tf_share_bps||5000)/100,settlements:settlements.results.filter((item:any)=>item.partner_id===row.id).map((item:any)=>({period:item.period,grossCommission:operations.filter(operation=>(operation.partnerId===row.id||operation.origin.localeCompare(row.name,'pt-BR',{sensitivity:'base'})===0)&&operation.date.startsWith(item.period)).reduce((sum,operation)=>sum+operation.grossCommission,0),feeRate:(item.fee_rate_bps||201)/100,tfShare:(item.tf_share_bps||5000)/100,status:item.status||'em_aberto',paidAt:item.paid_at||''})),adjustments:adjustments.results.filter((item:any)=>item.partner_id===row.id).map(normalizedGgAdjustment),bonuses:bonuses.results.filter((item:any)=>item.partner_id===row.id).map((item:any)=>({period:item.period,value:(item.value_cents||0)/100,description:item.description||'Bonificação'}))}))});
}
